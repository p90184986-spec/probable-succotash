...
This is a static site and does not require npm, a framework, or a build step. The included `vercel.json` explicitly disables install/build commands and serves the project root as static output.
...
The `vercel.json` file also adds the same basic security headers used by the local Apache configuration.
...
The newsletter panel uses a direct email link rather than collecting addresses. Connect a mailing platform only after defining the audience, consent process, privacy notice, and unsubscribe workflow.
